  ...
  fun str -> 
   ...
   then let (s,str') = scanId str
     in let token =
       ...
       else if s = "write" then WriteT
       else if s = "read" then ReadT
       else if s = "if" then IfT
       else if s = "while" then WhileT
       else if s = "expand" then ExpandT
       else if s = "variable" then VariableT
       else if s = "array" then ArrayT
       else if s = "alias" then AliasT
       else if s = "macro" then MacroT
       else IdT s
     in (token :: scan str')
   else match c with
     | ';' -> SemicT :: (scan str1)
  ...
  fun tokens -> 
   ...
   | WriteT :: tokens1 ->
      let (exp, tokens') = parseExp tokens1
       in (WriteC exp, tokens')
   | ReadT :: tokens1 ->
      let (left, tokens') = parseLeft tokens1
       in (ReadC left, tokens')
   | IfT :: tokens1 -> 
   ...
   | token1 :: tokens1 -> 
      (let (id, tokens2) = getIdT tokens1 in
       ...
       | MacroT ->
          ...
          in (MacroD (id,body), expectToken RcurlyT tokens3)
